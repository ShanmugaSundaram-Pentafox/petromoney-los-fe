...
export function OverviewTab({ data, report }) {
  ...
  activeAccs.forEach((a) => {
    const raw = a.accountTypeNormalized ?? a.accountType ?? 'Other';
    const k = String(raw).substring(0, 32);
  ...
  accs.forEach((a) => {
    const raw = a.accountTypeNormalized ?? a.accountType ?? 'Other';
    const t = String(raw).substring(0, 32);
  ...
  const lenderMap = {};
  accs.forEach((a) => {
    const l = a.memberShortName || 'Unknown';
    lenderMap[l] = (lenderMap[l] || 0) + 1;
  });
  ...
  const newest = [...accs]
    .filter((a) => a.dateOpened)
    .sort((a, b) => new Date(b.dateOpened) - new Date(a.dateOpened))[0];
          ...
          <Card withBorder radius="md" h="100%">
            ...
            <Stack gap="xs">
              {(scoreObj.reasonCodes || []).length ? (
                (scoreObj.reasonCodes || []).map((rc, i) => (
                  <ReasonCode
                    key={i}
                    code={rc.reasonCodeValue}
                    name={rc.reasonCodeName}
                  />
                ))
              ) : (
                <Text size="xs" c="dimmed">
                  No reason codes in response
                </Text>
              )}
            </Stack>
          </Card>
            ...
            <Stack gap={0}>
              {[
                { label: 'Total Accounts', value: accs.length },
                {
                  ...
                  value: accs.filter(isActive).length,
                },
                {
                  label: 'Closed Accounts',
                  value: accs.filter((a) => !isActive(a)).length,
                },
                {
                  label: 'Unique Lenders',
                  value: Object.keys(lenderMap).length,
                },
              ...
              {[
                {
                  ...
                  val: cleanDots,
                  pct: totalDots
                    ? ((cleanDots / totalDots) * 100).toFixed(1)
                    : 0,
                  color: 'green',
                },
                {
                  label: 'SMA',
                  val: smaDots,
                  pct: totalDots ? ((smaDots / totalDots) * 100).toFixed(1) : 0,
                  color: 'yellow',
                },
                {
                  label: 'DPD / Delinquent',
                  val: badDots,
                  pct: totalDots ? ((badDots / totalDots) * 100).toFixed(1) : 0,
                  color: 'red',
                },
                {
                  label: 'No Data (XXX)',
                  val: xxxDots,
                  pct: totalDots ? ((xxxDots / totalDots) * 100).toFixed(1) : 0,
                  color: 'gray',
                },
        ...
        <Grid.Col span={{ base: 12, sm: 6 }}>
          <Card withBorder radius="md" h="100%">
            <Text
              ...
              style={{ letterSpacing: '0.1em' }}
            >
              Active Credit Exposure Mix
            </Text>
            {totalBal > 0 ? (
              <Stack gap="sm">
                {Object.entries(mixMap)
                  .filter(([, v]) => v > 0)
                  .map(([k, v], i) => {
                ...
                <Group justify="space-between">
                  ...
                  <Text size="xs" fw={800} style={{ fontFamily: 'monospace' }}>
                    {fmtAmount(totalBal)}
                  </Text>
                </Group>
              </Stack>
