                    ...
                    <Table style={{marginBottom: 16}} key={i}>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell colSpan={3} align='center'>Account Holder Name</TableCell>
                                <TableCell colSpan={3} variant="body">{bank?.account_holder_name}</TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell>Bank Name</TableCell>
                                <TableCell colSpan={2} variant="body">{bank?.bank_name}</TableCell>
                                <TableCell>Account No.</TableCell>
                                <TableCell colSpan={2} variant="body">{bank?.account_no}</TableCell>
                                <TableCell colSpan={2}>Type of Account</TableCell>
                                <TableCell colSpan={2} variant="body">{bank?.account_type}</TableCell>
                            </TableRow>
                            <TableRow>
                        ...
                        <TableBody>
                            {
                                data?.bank_transaction_details?.filter(item => {return bank?.bank_number === item?.bank_number})?.map((row, i) => {
                                    return(
                                        <TableRow key={i}>
                                            <TableCell>{monthData.find(month => {return month.value == row.month})?.label} - {row.year}</TableCell>
                                            <TableCell><Currency value={row?.peak_util_lvl}/></TableCell>
                                            <TableCell><Currency value={row?.sanctioned_limit} /></TableCell>
                                            <TableCell>{row?.util_percent && row?.util_percent + '%'}</TableCell>
                                            <TableCell>{row?.no_of_inward_bounce}</TableCell>
                                            <TableCell>{row?.no_of_outward_bounce}</TableCell>
                                            <TableCell><Currency value={row?.bal_on_5th}/></TableCell>
                                            <TableCell><Currency value={row?.bal_on_15th}/></TableCell>
                                            <TableCell><Currency value={row?.bal_on_25th}/></TableCell>
                                            <TableCell><Currency value={row?.avg_mon_bal}/></TableCell>
                                        </TableRow>
                                    )
                                })
                            }
                        ...
                        <TableFooter>
                            <TableRow>
                                ...
                                <TableCell><Currency value={sumBy(data?.bank_transaction_details?.filter(item => {return bank?.bank_number === item?.bank_number}), 'peak_util_lvl')/6?.toFixed(2)} /></TableCell>
                                <TableCell colSpan={7} align='right'>ABB of last 6 months</TableCell>
                                <TableCell><Currency value={sumBy(data?.bank_transaction_details?.filter(item => {return bank?.bank_number === item?.bank_number}), 'avg_mon_bal')/6?.toFixed(2)} /></TableCell>
                            </TableRow>
                        </TableFooter>
                    </Table>
                )
